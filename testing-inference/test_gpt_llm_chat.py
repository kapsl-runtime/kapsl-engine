...
def get_gpt_model_id(forced_id=None):
    ...
    selected = select_model(models, ["gpt"], ["gpt-llm"])
    if not selected:
        print("No GPT model found active in kapsl-runtime.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Send chat prompts to a GPT model via kapsl-runtime."
    )
    ...
    model_id, model_name = get_gpt_model_id(args.model_id)
    if model_id is None:
        sys.exit(1)
