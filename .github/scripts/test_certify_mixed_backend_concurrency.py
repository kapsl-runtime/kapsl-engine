...
class MixedBackendMemoryTests(unittest.TestCase):
    def fixture(self, available: int = 1024, quarantine: int = 0) -> str:
        return "\n".join(
            [
                ...
                'kapsl_gpu_device_pool_owner_admitted{device="0",owner="gguf_kv:7"} 1',
                'kapsl_gpu_device_pool_owner_usage_bytes{device="0",owner="gguf_kv:7"} 256',
    ...
    def test_mixed_memory_requires_both_owners_and_ungranted_headroom(self):
        ...
        self.assertEqual(result["llama_owner"], "gguf_kv:7")
