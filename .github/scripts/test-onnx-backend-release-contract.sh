...
require_literal "$activator" 'manager.load_index()'
require_literal "$activator" '.select_compatible_pack(&index, &requirements, &target)'
require_literal "$activator" 'selection.manifest.adapter_abi.as_deref()'
require_literal "$activator" 'STANDARD_NATIVE_ADAPTER_ABI'
require_literal "$activator" '.ensure_pack(&selection.manifest)'
require_literal "$activator" 'activate_native_backend_pack(&selection.manifest, &installed)?;'
...
require_literal "$parity_certifier" 'sequence = ["baseline", "candidate", "candidate", "baseline"] * 10'
require_literal "$parity_certifier" '"readiness_poll_seconds": 0.005'
require_literal "$parity_certifier" '"max_startup_ratio": 1.5'
